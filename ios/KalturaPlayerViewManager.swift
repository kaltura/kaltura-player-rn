...
class KalturaPlayerViewManager: RCTViewManager {
    ...
    @objc func setMaxBitrate(_ bitrate: Int) {
        DispatchQueue.main.async {
            if self.kalturaPlayer != nil {
                self.player.preferredPeakBitRate = bitrate
            }
        }
    }
        ...
        kalturaPlayer.loadMedia(options: mediaOptions) { error in
            if (error != nil) {
                ...
                KalturaPlayerEvents.emitter.sendEvent(withName: "loadMediaFailed", body: [
                    "code": error?.code ?? "ConcurrencyLimitation",
                    "message": error?.localizedDescription ?? "Concurrency limitation",
                    "name": "OTTError",
