...
_defineProperty(KalturaPlayerAPI, "setup", async function (playerType, options) {
  ...
  if (!options && playerType != _consts.PLAYER_TYPE.BASIC) {
...
_defineProperty(KalturaPlayerAPI, "enableDebugLogs", function (enabled) {
  let logLevel = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _consts.LOG_LEVEL.DEBUG;
  if (enabled == null || logLevel == null) {
    return;
  }
  debugLogs = enabled;
  if (debugLogs === false || logLevel == _consts.LOG_LEVEL.OFF) {
    debugLogs = false;
    KalturaPlayerModule.setLogLevel(_consts.LOG_LEVEL.OFF);
  } else {
